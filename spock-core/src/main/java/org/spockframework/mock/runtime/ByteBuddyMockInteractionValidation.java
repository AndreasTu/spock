 ...
 * and the same validation is used for multiple mocks of the same class.</p>
 *
...
final class ByteBuddyMockInteractionValidation implements IMockInteractionValidation {
  ...
  private void validateProperty(EqualPropertyNameConstraint propNameConstraint) {
    ...
    validateNonFinalMethod(propertyToMethodName(GET, propName));
    validateNonFinalMethod(propertyToMethodName(IS, propName));
  }
